...
class DetectionMarkerFusionNode(Node):
    # 목적: YOLO 감지 결과와 LiDAR 스캔을 융합하여 RViz용 마커를 생성. 위험 위치 추적.
    # 연결: /detections, /scan 토픽 구독. TF로 좌표 변환, markers 퍼블리시.
    def __init__(self):
        ...
        self.latest_scan = None  # 설명: 최신 LiDAR 스캔 데이터. 범위 계산에 사용.
        self.markers = MarkerArray()  # 설명: RViz 마커 배열. 퍼블리시 대상.
        self.marker_id = 0  # 설명: 마커 ID 카운터. 고유 ID 할당.
        self.danger_positions = {0: [], 1: []}  # 설명: 클래스별 위험 위치 리스트. 유턴 트리거.
        self.pending_positions = {0: [], 1: []}  # 설명: 클래스별 보류 위치 리스트. 확인 대기.
        self.is_turning = False  # 설명: 턴 상태. 감지 처리에 영향.
        self.cmd_turning = False  # 설명: 명령 기반 턴 상태.
        self.turn_ignore_until = 0.0  # 설명: 턴 무시 타이머.
        self.last_turn_block_log_time = 0.0  # 설명: 마지막 로그 시간.
        self.tf_warned = False  # 설명: TF 경고 플래그.
